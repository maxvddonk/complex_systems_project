...
class MHSampler:
    def __init__(self, lattice_size = (50, 50), temperature = 293.15, J = 1):
...
df = pd.DataFrame(columns = [str(x) for x in range(50*50)] + ["temperature"])

sampler = MHSampler(lattice_size = (50, 50))

t_max = float(input("Maximum T:"))

t_min = 2.26918531421 - (t_max - 2.26918531421)
for t in [t_max, t_min]:
    temperature = t
    for _ in range(100):
        lattice = sampler.generate_sample(temperature, iterations = 50 * 50 * 10)
